...
#' lhtext
#'
#' Simple way to create word document using loop from list t.
#' Type lhtext and copy the template to R workspace and start writing.
...
#' TEMPLATE
#' t<-NULL
#' t[[1]]<-c("lev1","Ceci est un test. lev1= level 1 title")
#' t[[2]]<-c("txtn","bold","Add bold paragragh with page break","pageb") #normal simple text
#' t[[3]]<-c("lev2","Title for level 2. Below is for creating table")
#' tab0<-data.frame(x=c("MONDAY","TUESDAY"),y=c("EAT SOMETHING","DON'T EAT"))
#' t[[4]]<-c("tab","FlexTable(tab0)")
#' t[[5]]<-c("fnt","This is foot not with font size of 9 by default","pageb")
#' t[[6]]<-c("lev2","This a test Plot- Used plot. ggplot could be inserted")
#' t[[7]]<-c("fig","plot(1,1)",5,5,"pageb")
#' tab1<-read.csv("1-OH-Midazolam1.25+1.25mg.csv")
#' t[[8]]<-c("lev2","Another Table from folder")
#' t[[9]]<-c("tab","FlexTable(tab1)","pageb")
#' t[[10]]<-c("lev2","This a test Image imported from folder and inserted in doc")
#' t[[11]]<-c("ima","hydroxy-auc0.6-v-age-1.png",7.4,6)
#' t[[12]]<-c("txtc","This is complex text editor is ::b:i"," X","2::e:u" ,"bold::b:i:s","fwb::e","this::i","fsti::b:i") #formatted text.
...
lhtext <-function(t,toc.level=3,template="C:/Users/User/Desktop/packdev/styleapp.docx",TOC=T,
                 style.to.map=c("Heading1", "Heading2","Heading2","fnt"), working.folder=NULL,logo.certara="none"){
...
for(i in 1:length(t)){
    ...
    if(t[[i]][1]=="txtn"){
      if(t[[i]][2]=="bold"){
      txt1<-pot(t[[i]][2], textProperties(font.weight='bold'))}else{txt1<-t[[i]][2]}
      doc <-addParagraph(doc,txt1)}

    if(t[[i]][1]=="fnt"){
      doc <-addParagraph(doc,t[[i]][2], stylename ="fnt")}
    if(t[[i]][1]=="fig"){
      body(b)<-parse(text =t[[i]][2])
      doc <-addPlot(doc,fun = function() b(),
                    width = as.numeric(t[[i]][3]), height = as.numeric(t[[i]][4]),
                    par.properties = parProperties(text.align = "center"))
    }
    if(t[[i]][1]=="ima"){
      doc <-addImage(doc,t[[i]][2],
                     par.properties = parProperties(text.align = "center"),
                     width =as.numeric(t[[i]][3]), height =as.numeric(t[[i]][4]))
    }
    if(t[[i]][1]=="tab"){
      body(b)<-parse(text =t[[i]][2])
      doc <-addFlexTable(doc,b(),parProperties(text.align = "center"))
    }
    ...
    if(t[[i]][1]=="txtc"){
      ...
      for(j in 2:length(c)){
        ...
        z11<-c(z11,z1)
        if(length(grep(":b",sub(".*:b", ":b", c[j])))!=0){
          z2="bold"}else{z2="normal"}
        z22<-c(z22,z2)
        z3=ifelse(length(grep(":s",sub(".*:s", ":s", c[j])))!=0,
          "subscript",
        ifelse(length(grep(":e",sub(".*:e", ":e", c[j])))!=0,
          "superscript","baseline"))
        z33<-c(z33,z3)
        if(length(grep(":u",sub(".*:u", ":u", c[j])))!=0){
          z4=T}else{z4=F}
        z44<-c(z44,z4)
        if(length(grep("::",sub(".*::", "::", c[j])))==0){
          c1<-c[j]}else{c1<-gsub(sub(".*::", "::",c[j]),"",c[j])}
        cc<-c(cc,c1)
        all[[j]]<-paste0("font.size = 12 ,font.weight =z22[",j,"],font.style =z11[",j,"],underlined =",z44[j],",vertical.align =z33[",j,"])")}
      z1<-""
      for(j in 2:(length(c))){
        if(j<length(c)){
          z1<-paste0(z1,paste0("pot(cc[",j,"],textProperties(",all[[j]],")+"))
        }else{z1<-paste0(z1,paste0("pot(cc[",j,"],textProperties(",all[[j]],")"))}
      }
      b <- function(x) {}
      body(b)<-parse(text =z1)
      text<-b()
